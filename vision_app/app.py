...
#tạo folder với lại mấy cái list linh tinh
...
def determine_waste_type(labels):
    ...
    for label in labels:
        ...
        if any(word in description for word in organic_labels): return 'Hữu cơ'
        elif any(word in description for word in inorganic_labels): return 'Vô cơ'
        elif any(word in description for word in electronic_labels): return 'Linh kiện điện tử'
    return 'Khác'
#này là chỗ liên kết với index.html
...
def index():
    if request.method == 'POST':
        ...
        if (file and file.filename and allowed_file(file.filename)) or (url and url.strip()):
            ...
            session['waste_type'] = determine_waste_type(labels)
            return redirect(url_for('result'))
    return render_template('index.html')
#này là chỗ liên kết với cái result.html
@app.route('/result')
def result():
...
def detect_labels_and_objects(path):
    ...
    # cái này rename lại ảnh đã xử lý
